...
def calculate_eligibility_for_pl(mentor: PraktikumsLehrkraft) -> list:
    ...
    for p_type in applicable_types:
        ...
        if p_type.is_block_praktikum:
            ...
            allowed_subjects = get_allowed_subject_codes(mentor.program, p_type.code)

            for subject in mentor.available_subjects.all():
                # --- NEW: CURRICULUM VALIDITY CHECK ---
                if subject.code not in allowed_subjects:
                    # Example: Mentor has 'AL' but Program is 'GS'.
                    # 'AL' is not in get_allowed_subject_codes('GS', 'SFP').
...
def _calculate_available_pls_for_demand_item(demand_item):
    ...
    eligible_pls_query = PraktikumsLehrkraft.objects.filter(
        is_active=True, program=demand_item["program_type"]
    )
...
def _count_pls_for_block_praktikums():
    ...
    return (
        PraktikumsLehrkraft.objects.filter(
            is_active=True, available_praktikum_types__code__in=["PDP_I", "PDP_II"]
        )
        .distinct()
        .count()
    )
...
def _count_pls_for_wednesday_praktikums():
    ...
    return (
        PraktikumsLehrkraft.objects.filter(
            is_active=True, available_praktikum_types__code__in=["SFP", "ZSP"]
        )
        .distinct()
        .count()
    )
...
def _validate_assignment_rules(mentor, assignments: list, force_override: bool):
    ...
    if not force_override:
        if mentor.capacity == 2 and len(assignments) == 2:
...
def _update_mentor_assignments(mentor, assignments: list) -> list:
    """Internal helper: Handles database transaction, cleanup, and creation."""
